...
import { LayoutDashboard, Users, FileText, Package, BarChart2 } from 'lucide-react'
...
const TABS = [
  ...
  { id: 'reports',   icon: BarChart2,      label: 'Reports',   labelHi: 'रिपोर्ट' },
]

export default function BottomNav({ activeTab, onTabChange }) {
  const { language, stats, invoices } = useApp()
  const hi = language === 'hi'
  const unpaidCount = invoices?.filter(i => i.status !== 'paid').length || 0
  ...
  return (
    <nav style={{
      ...
      {TABS.map(tab => {
        ...
        const badge = tab.id === 'inventory' && stats.lowStockCount > 0
          ? stats.lowStockCount
        ...
        return (
          <button key={tab.id} onClick={() => onTabChange(tab.id)} style={{
            ...
            {active && (
              <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 28, height: 3, borderRadius: '0 0 4px 4px', background: 'linear-gradient(90deg, var(--saffron), #FF8C42)' }} />
            )}
            {badge && (
              <div style={{ position: 'absolute', top: 6, right: '18%', minWidth: 16, height: 16, borderRadius: 99, background: 'var(--red)', border: '1.5px solid white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 800, color: 'white', padding: '0 3px' }}>
                {badge > 9 ? '9+' : badge}
              </div>
            )}
