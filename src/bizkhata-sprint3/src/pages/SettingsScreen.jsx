...
import { User, Building2, Globe, Shield, Bell, HelpCircle, LogOut, ChevronRight, Crown, Check, Users, Download, FileSpreadsheet } from 'lucide-react'
import { exportOutstandingCSV, exportToCSV } from '../utils/pdfGenerator'

export default function SettingsScreen({ onNavigate }) {
  const { currentUser, logout, business, setBusiness, language, setLanguage, parties, transactions } = useApp()
  const hi = language === 'hi'
  const [showBusinessModal, setShowBusinessModal] = useState(false)
  const [showPlanModal, setShowPlanModal] = useState(false)

  const handleExportOutstanding = () => exportOutstandingCSV(parties, 'bizkhata-outstanding')
  const handleExportTransactions = () => {
    ...
    exportToCSV(data, 'bizkhata-transactions')
  }
  ...
  const sections = [
    {
      ...
      items: [
        ...
        { icon: <Users size={18} color="var(--indigo)" />, label: hi ? 'स्टाफ प्रबंधन' : 'Staff Management', sub: hi ? 'टीम और भूमिकाएं' : 'Manage team & roles', action: () => onNavigate && onNavigate('staff') },
    ...
    {
      title: hi ? 'डेटा एक्सपोर्ट' : 'Data Export',
      items: [
        { icon: <Download size={18} color="var(--green)" />, label: hi ? 'बकाया रिपोर्ट CSV' : 'Export Outstanding CSV', sub: hi ? 'सभी बकाया पार्टियां' : 'All parties with balance', action: handleExportOutstanding },
        { icon: <FileSpreadsheet size={18} color="var(--green)" />, label: hi ? 'लेनदेन CSV' : 'Export Transactions CSV', sub: hi ? 'सभी लेनदेन' : 'Full transaction history', action: handleExportTransactions },
      ]
    },
    {
          ...
          <div>
            ...
            <div style={{ marginTop: 6 }}>
              <span style={{
                ...
                borderRadius: 99, padding: '3px 12px', fontSize: 12, color: 'white', fontWeight: 600,
              }}>
      ...
      <div style={{ padding: '20px 16px 0', display: 'flex', flexDirection: 'column', gap: 20 }}>
        {sections.map(section => (
          <div key={section.title}>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.8px', textTransform: 'uppercase', marginBottom: 10 }}>
              {section.title}
            </p>
              ...
              {section.items.map((item, i) => (
                <div key={i} onClick={item.action}
                  style={{
                    ...
                    background: item.highlight ? 'linear-gradient(135deg, #FFFBEB, #FEF3C7)' : 'white',
                  }}
                ...
                >
                  <div style={{
                    ...
                    background: item.highlight ? 'var(--amber-light)' : 'var(--bg)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                  }}>
                    {item.icon}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 14, color: item.highlight ? 'var(--amber)' : 'var(--text-primary)' }}>
                      {item.label}
                    </div>
        ...
        <button onClick={logout} className="btn-ghost" style={{
          width: '100%', justifyContent: 'center', color: 'var(--red)',
        ...
        <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--text-muted)', paddingBottom: 8 }}>
          BizKhata v1.0.0 · Made with ❤️ in India
        </p>
      </div>
...
function PlanModal({ onClose }) {
  const features = [
    ...
    { label: 'Staff accounts', free: '1 (Owner)', pro: 'Up to 10' },
    { label: 'Products', free: 'Up to 30', pro: 'Unlimited' },
    { label: 'Reports & PDF export', free: 'Basic only', pro: 'All reports' },
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'flex-end', backdropFilter: 'blur(4px)' }}
      ...
      <div className="card slide-up" style={{ width: '100%', borderRadius: '24px 24px 0 0', padding: '24px 20px 40px', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 800, color: 'var(--indigo)' }}>
            Upgrade to Premium 👑
          </h3>
        ...
        <div style={{ background: 'linear-gradient(135deg, var(--indigo), var(--indigo-mid))', borderRadius: 16, padding: '20px', marginBottom: 20, textAlign: 'center' }}>
      ...
      <div className="card slide-up" style={{ width: '100%', borderRadius: '24px 24px 0 0', padding: '24px 20px 40px', maxHeight: '85vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700, color: 'var(--indigo)' }}>
            {hi ? 'बिज़नेस प्रोफाइल' : 'Business Profile'}
          </h3>
          <button onClick={onClose} style={{ background: 'var(--bg)', border: 'none', borderRadius: 10, width: 32, height: 32, cursor: 'pointer', fontSize: 18 }}>✕</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[
            ...
            { key: 'gstin', label: 'GSTIN (optional)', labelHi: 'GSTIN (वैकल्पिक)', placeholder: '24AAXXX1234F1Z5' },
          ].map(f => (
            <div key={f.key}>
          ...
          <button className="btn-primary" style={{ width: '100%', marginTop: 4 }}
            onClick={() => { setBusiness(form); onClose() }}>
