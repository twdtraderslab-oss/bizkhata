...
import { Package, AlertTriangle, Plus, TrendingUp, TrendingDown, ArrowLeft, BarChart2, MessageCircle, Download } from 'lucide-react'
import { exportOutstandingCSV } from '../utils/pdfGenerator'
      ...
      <div style={{ padding: '16px 16px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ padding: '14px 16px 10px', borderBottom: '1px solid var(--border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700, color: 'var(--indigo)' }}>
              📱 {hi ? 'बल्क WhatsApp रिमाइंडर' : 'Bulk WhatsApp Reminders'}
            </h3>
            <span style={{ background: 'var(--green-light)', color: 'var(--green)', borderRadius: 99, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>
              {parties.filter(p => p.balance > 0 && p.balanceType === 'to_receive' && p.phone).length} {hi ? 'पार्टियां' : 'Parties'}
            </span>
          </div>
          <div style={{ padding: '12px 16px' }}>
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>
              {hi ? 'सभी बकायेदारों को एक साथ WhatsApp भेजें' : 'Send payment reminders to all outstanding customers at once'}
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {parties
                ...
                .slice(0, 5)
                .map(p => (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg)', borderRadius: 10, padding: '10px 12px' }}>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>{p.name}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>₹{p.balance.toLocaleString('en-IN')} pending</div>
                    </div>
                    <button
                      onClick={() => {
                        const msg = encodeURIComponent(`Namaste ${p.name} ji,\n\nYour payment of *₹${p.balance.toLocaleString('en-IN')}* is pending with us.\n\nKindly arrange payment at earliest.\n\nThank you 🙏`)
                        window.open(`https://wa.me/91${p.phone}?text=${msg}`, '_blank')
                      }}
                      style={{
                        background: '#25D366', border: 'none', borderRadius: 8, padding: '6px 12px',
                        color: 'white', fontWeight: 700, fontSize: 12, cursor: 'pointer',
                        display: 'flex', alignItems: 'center', gap: 5,
                      }}>
                      <MessageCircle size={13} /> {hi ? 'भेजें' : 'Send'}
                    </button>
                  </div>
                ))
              }
            </div>
            {parties.filter(p => p.balance > 0 && p.balanceType === 'to_receive' && p.phone).length > 5 && (
              <p style={{ fontSize: 12, color: 'var(--text-muted)', textAlign: 'center', marginTop: 10 }}>
                + {parties.filter(p => p.balance > 0 && p.balanceType === 'to_receive' && p.phone).length - 5} more parties
              </p>
            )}
          </div>
        </div>
        ...
        {/* Export Data */}
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => exportOutstandingCSV(parties, 'bizkhata-outstanding')} className="btn-ghost" style={{
            flex: 1, justifyContent: 'center', color: 'var(--green)', border: '1px solid var(--green-light)', background: 'var(--green-light)', fontWeight: 700,
          }}>
            <Download size={15} /> {hi ? 'CSV एक्सपोर्ट' : 'Export CSV'}
          </button>
        </div>
